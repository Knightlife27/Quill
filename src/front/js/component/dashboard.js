...
//     try {
//       const response = await fetch(`${BACKEND_URL}/api/dashboard/${encodeURIComponent(dashboardName)}`);
//       if (!response.ok) {
...
const Dashboard = ({ containerStyle, onClickDashboardItem }) => {
  ...
  const [dashboard, setDashboard] = useState(null);
  const [initialData, setInitialData] = useState([]);
  const [initialFetchRange, setInitialFetchRange] = useState({}); // Track the initial fetch range
  const [filteredData, setFilteredData] = useState([]);
  const [dateRange, setDateRange] = useState({
    startDate: subDays(new Date(), 90),
    endDate: new Date()
  ...
  const fetchDashboard = async (dashboardName) => {
    try {
      ...
      setInitialData(data.charts);
      setFilteredData(data.charts);
      setInitialFetchRange(dateRange); // Set the initial fetch range
    } catch (error) {
      console.error('Error fetching dashboard:', error);
      setError('Failed to load dashboard. Please try again later.');
    }
  ...
  const handleDateChange = (newDateRange) => {
    ...
    setDateRange(newDateRange);
    if (dashboard && initialData.length > 0) {
      const isWithinInitialRange = newDateRange.startDate >= initialFetchRange.startDate && newDateRange.endDate <= initialFetchRange.endDate;
  ...
  const fetchNewData = async (newDateRange) => {
    try {
      ...
      console.log('Fetched New Data:', data);
      setDashboard(data);
      setInitialData(data.charts); // Update initial data with new fetch
      setFilteredData(data.charts); // Reset filtered data with new fetch
      setInitialFetchRange(newDateRange); // Update the initial fetch range
    } catch (error) {
      console.error('Error fetching new data:', error);
      setError('Failed to load new data. Please try again later.');
    }
